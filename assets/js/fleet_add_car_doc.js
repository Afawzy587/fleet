$('#AddDocBtn').click(function( event ) {
  ...
  if($("#add_doc_form").valid()){
    ...
    newDocItem.find('input[name="name[]"]')[0].value = docName;
    newDocItem.find('label[for="start_date[]"]')[0].innerText = date1Name;
    newDocItem.find('label[for="end_date[]"]')[0].innerText = date2Name;
    newDocItem.find('label[class*="image_upload_btn"]')[0].innerText = imgBtnName;
    newDocItem.find('label[for="value[]"]')[0].innerText = valueInputItem;
